...
- **Never overwrite `.env`** — it contains real server credentials; only read it. It is tracked in git intentionally (shared across all model branches) — do not "fix" that. `artifacts/` is gitignored runtime output.

## Branch layout — one branch per model run
