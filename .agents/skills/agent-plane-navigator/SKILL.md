...
type: prompt
whenToUse: When you need a targeted answer from PLANE.md (exact opcode, offset, field order, padding) without re-reading the whole spec in the main context.
---

Sub-agent role prompt. Dispatch a built-in **explore** sub-agent (read-only) and pass the body below as
its task brief. Converted from `.claude/agents/plane-navigator.md`.
