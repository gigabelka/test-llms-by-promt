...
type: prompt
whenToUse: After crypto-porter has driven the crypto self-tests to green, to build net/, login/game FSMs and index.ts up to IN_GAME (build-l2 steps 5-8).
---

Sub-agent role prompt. Dispatch a built-in **coder** sub-agent and pass the body below as its task brief.
Converted from `.claude/agents/fsm-builder.md`.
...
not your job — hand off to `agent-run-debugger`.
