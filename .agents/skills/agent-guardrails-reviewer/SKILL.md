---
name: agent-guardrails-reviewer
...
type: prompt
whenToUse: After a build or edit of src/, before running against the live server — read-only audit of src/ against the l2-guardrails checklist.
---

Sub-agent role prompt. Dispatch a built-in **explore** sub-agent (read-only) and pass the body below as
its task brief. Converted from `.claude/agents/guardrails-reviewer.md`.
