...
| ----- | ----- | ---------- |
| 1–2 (scaffold + `npm install`) | **orchestrator** (do this before calling `agent-crypto-porter`) | — |
| 3–4 (crypto + self-tests) | `agent-crypto-porter` | gate 1 green, then `agent-guardrails-reviewer` |
| 5–8 (net + login/game FSM + linear `index.ts`) | `agent-fsm-builder` | then `agent-guardrails-reviewer` |
| 9 (typecheck + run against server) | `agent-run-debugger` (via `run`/`debug-l2`) | gate 2 |

So a from-scratch build is: orchestrator scaffolds → `agent-crypto-porter` (gate 1) → `agent-guardrails-reviewer` →
`agent-fsm-builder` → `agent-guardrails-reviewer` → `agent-run-debugger` (gate 2). Re-run `agent-guardrails-reviewer` after any fix
