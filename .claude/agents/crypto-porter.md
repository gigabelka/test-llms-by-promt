...
3. Wire `runLoginCryptoSelfTests()` + `runGameCryptoSelfTests()` and run the round-trips:
   `blowfishDecrypt(blowfishEncrypt(x,k),k).equals(x)` and `gameCrypt.decrypt(gameCrypt.encrypt(x)).equals(x)`
   (two instances, one 8-byte key, `enabled=true`, static tail `c8 27 93 01 a1 6c 31 97`).

Sockets, `net/`, FSM — **not your scope**, don't touch.

## Done criterion

`npx tsc --noEmit` clean **AND** both round-trips green. A red self-test = crypto pasted
