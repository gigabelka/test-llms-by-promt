...
|-------|-------------|-----------|
| **crypto-porter** | Read, Write, Edit, Bash, Grep | Портирует крипто-модули (Blowfish, NewCrypt, LoginCrypt, GameCrypt, RsaCrypt) и `debug/DebugTools.ts` дословно из PLANE.md и доводит крипто self-tests до зелёного (гейт 1) до любой работы с сокетами. Вызывать, когда `src/crypto` пуст/подозрителен или round-trip тесты красные. |
